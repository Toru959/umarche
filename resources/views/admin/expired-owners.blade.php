                ...
                <div class="container px-5 mx-auto">
                    ...
                    <table class="table-auto w-full text-left whitespace-no-wrap">
                        ...
                        <tr>
                            ...
                            <td class="border-t-2 border-gray-200 px-4 py-3">{{$owner->deleted_at}}</td>

                            <td class="px-4 py-3">        
                            <form id="delete_{{$owner->id}}" method="post" action="{{route('admin.expired-owners.destroy', ['owner'=>$owner->id])}}">
                            @csrf
                            <td class="border-t-2 border-gray-200 w-10 text-center">
                            <a href="#" data-id="{{$owner->id}}" onclick="deletePost(this)" class="text-white bg-red-500 border-0 py-2 px-8 focus:outline-none hover:bg-red-600 rounded text-lg">完全に削除する</a>
                            </td>
                            </form>
                        </tr>
                        @endforeach
